...
- [Testing Patterns](references/testing-patterns.md) -- EcotoneLite test patterns for aggregates: state-stored testing with `getAggregate()`, event-sourced testing with `withEventsFor()` and `popRecordedEvents()`, event store testing with `bootstrapFlowTestingWithEventStore()`, and multiple identifier testing. Load when writing tests for aggregates.
