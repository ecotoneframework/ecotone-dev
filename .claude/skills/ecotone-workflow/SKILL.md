...
- [Testing Patterns](references/testing-patterns.md) -- EcotoneLite test patterns for all workflow types: saga state testing with `getSaga()`, saga event testing with `getRecordedEvents()`, async saga testing with `run()`, saga `outputChannelName` testing, stateless workflow chain testing, async workflow testing, and orchestrator test setup (Enterprise). Load when writing tests for sagas, workflows, or orchestrators.
