...
final class Encoding
{
     ...
     */
    public static function binToHex(string $byte_string): string
    {
        $hex = '';
        $len = Core::strlen($byte_string);
        for ($i = 0; $i < $len; ++$i) {
     ...
     *
     * @throws BadFormatException|EnvironmentIsBrokenException
     *
     * @psalm-suppress TypeDoesNotContainType
     */
    public static function hexToBin(string $hex_string): string
    {
        $hex_pos = 0;
        $bin     = '';
        $hex_len = Core::strlen($hex_string);
        $state   = 0;
        $c_acc   = 0;
        ...
        while ($hex_pos < $hex_len) {
            ...
            if (($c_num0 | $c_alpha0) === 0) {
                throw new BadFormatException('Encoding::hexToBin() input is not a hex string.');
            }
    ...
    public static function trimTrailingWhitespace(string $string = ''): string
    {
        ...
        return (string) Core::substr($string, 0, $length);
    }
     ...
     *
     */
    public static function saveBytesToChecksummedAsciiSafeString(string $header, #[SensitiveParameter] string $bytes): string
    {
        // Headers must be a constant length to prevent one type's header from
        // being a prefix of another type's header, leading to ambiguity.
        Core::ensureTrue(Core::strlen($header) === self::SERIALIZE_HEADER_BYTES, 'Header must be ' . self::SERIALIZE_HEADER_BYTES . ' bytes.');

        return self::binToHex($header . $bytes . hash(self::CHECKSUM_HASH_ALGO, $header . $bytes, true));
    }

    /**
     * INTERNAL USE ONLY: Decodes, verifies the header and checksum, and returns the encoded byte string.
     *
     * @throws EnvironmentIsBrokenException|BadFormatException|CryptoException
     */
    public static function loadBytesFromChecksummedAsciiSafeString(string $expected_header, #[SensitiveParameter] string $string): string
    {
        // Headers must be a constant length to prevent one type's header from
        // being a prefix of another type's header, leading to ambiguity.
        Core::ensureTrue(Core::strlen($expected_header) === self::SERIALIZE_HEADER_BYTES, 'Header must be 4 bytes.');
        ...
        $bytes = self::hexToBin($string);

        // Make sure we have enough bytes to get the version header and checksum.
        if (Core::strlen($bytes) < self::SERIALIZE_HEADER_BYTES + self::CHECKSUM_BYTE_SIZE) {
            throw new BadFormatException('Encoded data is shorter than expected.');
        }

        // Grab the version header.
        $actual_header = (string) Core::substr($bytes, 0, self::SERIALIZE_HEADER_BYTES);

        if ($actual_header !== $expected_header) {
            throw new BadFormatException('Invalid header.');
        }

        // Grab the bytes that are part of the checksum.
        $checked_bytes = (string) Core::substr($bytes, 0, Core::strlen($bytes) - self::CHECKSUM_BYTE_SIZE);

        // Grab the included checksum.
        $checksum_a = (string) Core::substr($bytes, Core::strlen($bytes) - self::CHECKSUM_BYTE_SIZE, self::CHECKSUM_BYTE_SIZE);
        ...
        return (string) Core::substr($bytes, self::SERIALIZE_HEADER_BYTES, Core::strlen($bytes) - self::SERIALIZE_HEADER_BYTES - self::CHECKSUM_BYTE_SIZE);
    }
}
