...
class InterceptorOrderingCase
{
    ...
    #[After(precedence: -1, pointcut: self::POINTCUT, changeHeaders: true)]
    public function afterChangeHeaders(InterceptorOrderingStack $stack, array $metadata): array
    {
        $stack->add("afterChangeHeaders", $metadata);
        return [...$metadata, "afterChangeHeaders" => "header"];
    }

    #[After(pointcut: self::POINTCUT)]
    public function after(InterceptorOrderingStack $stack, array $metadata): InterceptorOrderingStack
    {
        return $stack->add("after", $metadata);
    }
}