...
use Test\Ecotone\Messaging\Fixture\HighThroughputPublishing\InMemoryPendingDelivery;
...
final class HighThroughputPublishingReliabilityTest extends TestCase
{
    ...
    public function test_future_of_delivery_flushed_by_backlog_limit_still_reports_failure(): void
    {
        ...
        $firstFuture = $publisher->publishDeferred('first order');
        for ($messageNumber = 0; $messageNumber < 1300; $messageNumber++) {
            $publisher->publishDeferred('unresolved order ' . $messageNumber);
        }
    ...
    public function test_unawaited_deliveries_of_all_registries_are_flushed_on_script_shutdown(): void
    {
        ...
        file_put_contents($scriptPath, <<<'PHP'
            ...
            $unawaitedDelivery = new class implements \Ecotone\Messaging\Channel\DeliveryConfirmation\PendingDelivery {
                private bool $awaited = false;

                public function awaitDelivery(): \Ecotone\Messaging\Channel\DeliveryConfirmation\DeliveryResult
                {
                    $this->awaited = true;
                    echo 'flushed;';

                    return \Ecotone\Messaging\Channel\DeliveryConfirmation\DeliveryResult::successful();
                }
    ...
    {
        $registry = new PendingDeliveryRegistry(new LoggingService());
    ...
    public function test_one_failing_batch_among_many_published_in_command_handler_rolls_back_transaction(): void
    {
        ...
        $commandHandler = new class () {
            ...
            public function handle(string $order, #[Reference(InMemoryHighThroughputPublisherModule::PUBLISHER_REFERENCE)] MessagePublisher $publisher): void
            {
                $publisher->publishDeferred(BatchMessage::constructEmpty()->append($order . ' first batch'));
                $publisher->publishDeferred(BatchMessage::constructEmpty()->append($order . ' poisoned batch'));
                $publisher->publishDeferred(BatchMessage::constructEmpty()->append($order . ' third batch'));
            }
        };
        $ecotoneLite = EcotoneLite::bootstrapFlowTesting(
            [$commandHandler::class, InMemoryHighThroughputPublisherModule::class, InMemoryHighThroughputOutboundAdapter::class, FakeTransactionModule::class],
