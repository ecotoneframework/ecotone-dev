...
{
    public function test_building_asynchronous_partitioned_projection(): void
    {
    ...
    public function test_operations_on_asynchronous_partitioned_projection(): void
    {
        $projection = $this->createAsyncProjection();
        ...
        $ecotone->deleteProjection($projection::NAME);
        self::assertFalse(self::tableExists($this->getConnection(), 'in_progress_tickets_async_partitioned'));
    }

    public function test_catching_up_events_after_reset_asynchronous_partitioned_projection(): void
    {
        $projection = $this->createAsyncProjection();
    ...
    public function test_asynchronous_partitioned_projection_runs_on_default_testing_pollable_setup(): void
    {
        $projection = $this->createAsyncProjection();
    ...
    public function test_asynchronous_partitioned_projection_runs_on_default_testing_pollable_setup_with_dbal_channel(): void
    {
        $projection = $this->createAsyncProjection();
    ...
    public function test_triggering_partitioned_projection_action_is_asynchronous(): void
    {
        $projection = $this->createAsyncProjection();
        ...
        self::assertFalse(
            ...
            'Projection deletion for PartitionedProjection is synchronous'
        );
    ...
    private function createAsyncProjection(): object
    {
        ...
        return new #[PartitionedProjection(self::NAME, partitionHeaderName: MessageHeaders::EVENT_AGGREGATE_ID), Asynchronous(self::CHANNEL), FromStream(stream: Ticket::class, aggregateType: Ticket::class)] class($connection) {
            ...
            public function getTickets(): array
            {
                return $this->connection->executeQuery(<<<SQL
                    SELECT * FROM in_progress_tickets_async_partitioned ORDER BY ticket_id ASC
            ...
            public function addTicket(TicketWasRegistered $event): void
            {
                $this->connection->executeStatement(<<<SQL
                    INSERT INTO in_progress_tickets_async_partitioned VALUES (?,?)
            ...
            {
                $this->connection->executeStatement(<<<SQL
                    DELETE FROM in_progress_tickets_async_partitioned WHERE ticket_id = ?
            ...
            {
                $this->connection->executeStatement(<<<SQL
                    CREATE TABLE IF NOT EXISTS in_progress_tickets_async_partitioned (
            ...
            {
                $this->connection->executeStatement(<<<SQL
                    DROP TABLE IF EXISTS in_progress_tickets_async_partitioned
                SQL);
            }
            ...
            {
                $this->connection->executeStatement(<<<SQL
                    DELETE FROM in_progress_tickets_async_partitioned
                SQL);
            }
        };
    }
    ...
    private function bootstrapEcotone(array $classesToResolve, array $services): FlowTestSupport
    {
        return EcotoneLite::bootstrapFlowTestingWithEventStore(
            ...
            runForProductionEventStore: true,
            enableAsynchronousProcessing: [
                SimpleMessageChannelBuilder::createQueueChannel('async_projection_partitioned'),
            ],
            licenceKey: LicenceTesting::VALID_LICENCE,
        );
    }
