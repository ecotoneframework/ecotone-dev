...
$eventStore = $app->get(EventStore::class);

echo "== Laravel Projection Quickstart - Eloquent Read Model ==\n\n";
...
$messagingSystem->runConsoleCommand('ecotone:projection:delete', ['name' => 'user_list_eloquent']);
echo "   Projection deleted\n\n";

echo "2) Initialise projection (create read model storage)\n";
$messagingSystem->runConsoleCommand('ecotone:projection:init', ['name' => 'user_list_eloquent']);
...
$messagingSystem->runConsoleCommand('ecotone:projection:backfill', ['name' => 'user_list_eloquent']);
$rows = $queryBus->sendWithRouting('user.listActive');
Assert::assertCount(1, $rows);
Assert::assertSame('Alice Cooper', $rows[0]['name']);
