...
4. **`#[Identifier]` goes on the property, not the getter.** For Doctrine entities, use the property-based `#[Identifier]` attribute — not `#[IdentifierMethod]` which is for Eloquent models with dynamic accessors.
